...
void shreyu(){
    ...
    int n2 = v2.size(); one++;  tp();
    int n1 = v1.size(); one++; tp();
    ...
    for(int i=0; i<n1/2; i++){
        one++; zero++; tp();
        int d = abs(v1[i] - v1[n1 - 1 - i]) / 2;
        ans2.pb(d); one++; zero++;
        ans1.pb(v1[i] + d); one++; zero++;
    }

    tp();
    for(int i=0; i<n2/2; i++){
        int d = abs(v2[i] - v2[n2 - 1 - i]) / 2; one++; zero++;
        ans2.pb(d); one++; zero++; tp();
        ans1.pb(v2[i] + d); one++; zero++;
    }

    zero++; one++; tp();
    for(auto i: ans1) cout << i << sp; 
    zero++; one++; tp();
    for(auto i: ans2) cout << i << sp; 
    zero++; one++; tp();
    one++; tp();
    line;
}
