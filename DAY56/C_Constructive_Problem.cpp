...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 
void shreyu(){
    ll n= 0, k= 0, c= 0, one= 0, zero= 0, mini= INT_MAX, MAXI= 0 ;
    string s = "" , temp = "";
    cin>>n    ;
    map<ll,ll> mp;
    map<ll,ll> mp1;
    ...
    ll req = mex + 1;
    // find left & right boundaries of req and make them mex;
    ll l = 0, r = 0;
    for(ll i = 0 ; i < n; i++){
        if(v[i] == req){
            l = i;
    ...
    if(mex == n ) nope;
    else if(mp[mex+1] == 0 && n > 2) yup;
    else if(mex == n - 1 && mp[n-2] > 1) yup;
    else if(mex1 == mex + 1) yup;
    else nope;
