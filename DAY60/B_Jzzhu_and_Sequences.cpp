...
void shreyu(){
    ...
    while(1) {
        ...
        a = b;
        b = c;
        pair<ll, ll> elementToSearch(a, b);

        auto it = find_if(vp.begin(), vp.end(), [&](const pair<ll, ll>& p) {
            return p.first == elementToSearch.first && p.second == elementToSearch.second;
        });
        if (it != vp.end()) break;
    }
