...
void shreyu(){
   ...
   cin>>n >>a>>b>>c>>d   ;
    ...
    int k = n/2;
    int m1 = max(min(abs(k-a), abs(k-a+1)), min(abs(k-b), abs(k-b+1)));
    int m2 = max(min(abs(k-c), abs(k-c+1)), min(abs(k-d), abs(k-d+1)));
    cout<<abs(m1-m2)<<endl;
    ...
    // cout<<abs(a-c)<<" ";
    // cout<<abs(b-d)<<endl;
    // cout<<min(abs(a-c), abs(b-d))<<endl;


    // int m1 = min(min(abs(a-c) , abs(n+1-a-c) ), abs(2*n + 2 - a - c)) ;
    // int m2 = min(min(abs(b-d) , abs(n+1-b-d) ), abs(2*n + 2 - b - d)) ;
    // // int m2 = min(abs(b-d) , abs(n+1-b-d) ) ;
