...
int distance( int i, v64 zero){
    ...
    int mini = INT_MAX, cu= 0, cl = 0;
    // for(int j = 0; j<n;j++){
    //     cu = upper_bound(all(zero) , )
    //     // c = abs(zero[j] - i);
    //     mini = min(mini , c);
    // }    

    auto lo = lower_bound(zero.begin(), zero.end(), i);
    auto hi = upper_bound(zero.begin(), zero.end(), i);
    ...
    // cout<<abs(cu);
    // cout<<"--";
    return min(abs(i-zero[cl]), abs(i-zero[cu]));
    
}

void shreyu(){
   //input ->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 
   int n , c = 0 , one = -1 , two = -1 ;
   string s = "" , temp = "";
   cin>>n    ;
   v64 v , ans , zero ;
   forn(i,n){
   ...
   //code->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    for (ll i=0 ; i<n ; i++)
    {
        if (curr == 0){ cout << abs(i-zero[curr]) << ' ';}
        else {cout << min(abs(i-zero[curr]) , abs(i-zero[curr-1]) ) << ' ';}
        if (v[i] == 0) {curr++;if (curr == zero.size()){curr--;}}
    }
