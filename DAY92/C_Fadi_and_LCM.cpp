...
ll lcm(ll a, ll b) {
    ll g = gcd(a, b);
    return (a / g) * b;  // Divide a first to reduce the chance of overflow
}

int ans1 = 1000000000000, ans2= 1000000000000;
void rec(v64 &v, int n, int idx, int a1, int a2, int d) {
    // if(a1 > ans1 || a2 > ans2) return;
    if(idx == n) {
        int lc = lcm(a1, a2);
        if(lc != d) return;
        if(a1 == a2) return;
        if(max(ans1, ans2) > max(a1, a2)) {
            ans1 = a1;
            ans2 = a2;  
        }
        return;
    ...
    rec(v, n, idx+1, a1*v[idx], a2, d);
    rec(v, n, idx+1, a1, a2*v[idx], d);
}
...
void shreyu(){
    ...
    ans1 = 1000000000000, ans2=1000000000000;
    rec(v, v.size(), 0, 1, 1, n);
