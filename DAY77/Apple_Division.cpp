...
void ff(v64 &v, ll &n, ll zero, ll sum, ll idx) {
    if(idx == v.size()) return ;
    // if(sum == zero) return true;
    // if(sum > zero) return false;
    global.pb(sum);
    ff(v, n, zero, sum, idx + 1);
    // if(h1) return h1;
    sum += v[idx];
    global.pb(sum);
    ff(v, n, zero, sum, idx + 1);
...
//  void f(v64 &v, ll &n, ll zero, ll sum) {
//      ff(v, n, zero, sum, 0);
//  }
...
void shreyu(){
    ...
    for(int i=0; i<global.size(); i++) {
        ll nx = one - global[i];
        mini = min( mini, abs(nx - global[i]) ); 
    }
    

    
    cout<<mini<<endl;
    // cout<<abs(zero - nx)<<endl;

    // cout << abs( (one/2 - zero) - (one - (one/2 - zero))) <<endl;  
