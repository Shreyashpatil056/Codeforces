 ...
 #include <bits/stdc++.h> 
 // This isnt working for 7th test case yet
 using namespace std;
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 
void shreyu(){
    ll n= 0, k= 0, d = 0 ,c= 0, one= 0, zero= 0, mini= INT_MAX, MAXI= 0 ;
    string s = "" , temp = "";
    cin>>n    ;
    vector<string> v ;
    ...
    bool possi2 = false;
    bool corner = true;
    bool leftmaidaalo = true;

    vp64 danger;

    //here zero is total , one is just count of brackets + - both
    forn(i,n){
        possi = false;
        cin>>s;
        ll nn = s.size();
        ...
        for(ll i = 0 ; i < s.size() ; i++ ) {
            ...
            if(isPair(vc[vc.size()-1], s[i] )) vc.erase(vc.begin() + vc.size() - 1);
        ...
        if(one > 0 && possi) possi2 = true;  
        v.pb(s);
        if(one != 0) corner = false;
        zero += one;
        mp[i] = one;
        one = 0;
        ...
        if(s.size() == 0){
            ...
            if(s[0] == '(') danger.pb({s.size() , i+1});
            else danger.pb({-1*s.size(), i+1});
        }
    }
    ...
    v64 ans;
    for(auto i : danger) ans.pb(i.se - 1);
