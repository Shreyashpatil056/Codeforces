...
void shreyu() {
    ...
    for (auto [x, i] : pos) {
        if (have_a.count(x) && *have_b.begin() <= x) {
            res_a[i] = x;
            res_b[i] = *have_b.begin();
            have_a.erase(x);
            have_b.erase(have_b.begin());
        } else if (have_b.count(x) && *have_a.begin() <= x) {
            res_b[i] = x;
            res_a[i] = *have_a.begin();
            have_b.erase(x);
            have_a.erase(have_a.begin());
        } else {
            cout << "NO\n";
            return;
        }
