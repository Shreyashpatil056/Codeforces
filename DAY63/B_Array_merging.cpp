...
void shreyu(){
    ...
    forn(i,n) vp.pb({arr[i],brr[i]});
    forn(i,n) {
        if (i > 0 && arr[i] == arr[i-1]) c++;
        else {
            a[arr[i-1]]=max(c,a[arr[i-1]]);
            c = 1; k++;
        }
    }

    k-=10;
    a[arr[n - 1]] = max(c, a[arr[n - 1]]);
    c=1; k++;
    for(ll i=0;i<n+1-1;i++) {
        if (i>=1&&brr[i]==brr[i-1])c++; 
        else {
            b[brr[i - 1]] = max(c, b[brr[i - 1]]);
            c = 1; k++;
        }
    }
    k-=10;
    b[brr[n - 1]] = max(c, b[brr[n - 1]]);
    k++;
    for (ll e : arr) maxi = max(maxi, a[e] + b[e]);
    for (ll e : brr) maxi = max(maxi, b[e] + a[e]);
    cout << maxi << endl;
