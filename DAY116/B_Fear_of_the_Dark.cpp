...
ld dis(ld x, ld y, ld x1, ld y1){
    return sqrt(pow(x - x1, 2) + pow(y - y1, 2));
}
 
bool f(ld x, ld y, ld x1, ld y1, ld x2, ld y2, ld mid){
    ld d1, d2, d;
    ld d11, d22 ,d0, bd;
    d1  = dis(x,  y,  x1, y1);
    d11 = dis(0,  0,  x1, y1);
    ...
    if(d1 > rad && d2 > rad) return false;

    if(d1 <= rad){
        if(d11 <= rad) return true;
    }

    if(d2 <= rad){
        if(d22 <= rad) return true;
    }

    if(bd <= 2 * rad){
        if((d1 <= rad && d22 <= rad) || (d2 <= rad && d11 <= rad)) {
            return true;
        }
    }
...
void shreyu(){
    ...
    ld start = 0, end = 1000000, ans = -1;
    while(start <= end){
        ld mid = (start + end) / 2;
        // dbg(mid);
        if(f(x, y, x1, y1, x2, y2, mid)){
