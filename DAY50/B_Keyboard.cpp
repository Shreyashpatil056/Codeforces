...
void shreyu(){
   int n ,m,x, c = 0 , one = 0 , zero = 0 ;
   string s = "" , temp = "";
   cin>>n >>m>>x   ;

//    char arr[n][m];
   vector<vector<char>> arr((n,m));
   vp64 vp;
   ...
   forn(i,n){
    ...
    // for(int ve = 0; ve <n ; ve++){
        ...
        //     forn(j,m){
        //         if(mp[arr[i][j]]==0){   
        //             mp[arr[i][j]] = abs(i+j-sum);
        //         }
        //         mp[arr[i][j]] = min(mp[arr[i][j]],abs(i+j-sum) );
        //     }
        // }
    // }
    ...
    for (int i = 0; i < arr.size(); i++) {
        for (int j = 0; j < arr[i].size(); j++) {
            if (arr[i][j] == 'S') {
                // update minimum distance for aint lowercase characters in the same row
                for (char c = 'a'; c <= 'z'; c++) {
mp[c] = min(mp[c], static_cast<int>(abs(j - distance(arr[i].begin(), find(arr[i].begin(), arr[i].end(), 'S')))));
                }
            }
        }
    }
    ...
    for(int i=0;i<sz; i++){
        if(isupper(s[i])){
            char ch = s[i];
            ch += 32;
            // cout<<ch<<sp<<mp[ch]<<endl;
            if(mp[ch] > x){
                c++;
                // cout<<ch<<sp<<i<<sp<<mp[ch]<<endl;
            }
        }
    }
