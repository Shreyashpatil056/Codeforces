...
// PRINT PRIME FACTORS 
vll ppf(ll n){vll v;while(n%2==0){v.pb(2);n=n/2;}for(ll i=3;i<=sqrt(n);i=i+2){while(n%i==0){v.pb(i);n=n/i;}}if(n>2){v.pb(n);}return v;}
...
vll primepushVector(vll v, ll n) { ll chk=0; for(ll i=1; i<=n; i++) { for(ll j=2; j<i; j++) { if(i%j==0) { chk++; break; } } if(chk==0 && i!=1) v.push_back(i); chk = 0; } return v; }
