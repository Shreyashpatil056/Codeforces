...
//<=====================================================================>
void shreyu(){
    ll one = 0, zero = 0, mini = INT_MAX, maxi = -INF ;
    ll odd = 0, even = 0 ;
    ll n = 0, m = 0, k = 0, c = 0;
    // string s = "" , temp = "";
    cin >> n  ;
    ...
    maxi = max(maxi, v[0] * v[1] * v[2] * v[3] * v[n-1]);
    maxi = max(maxi, v[0] * v[1] * v[2] * v[n-2] * v[n-1]);
    maxi = max(maxi, v[0] * v[1] * v[n-3] * v[n-2] * v[n-1]);
    maxi = max(maxi, v[0] * v[n-4] * v[n-3] * v[n-2] * v[n-1]);
    maxi = max(maxi, v[n-5] * v[n-4] * v[n-3] * v[n-2] * v[n-1]);
    
    if(zero) maxi = max(maxi, 0ll);
