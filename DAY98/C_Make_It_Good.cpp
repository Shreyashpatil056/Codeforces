...
void shreyu(){
    ...
    while(s >= 0 && v[s] >= v[s+1]) s--;
    while(s >= 0 && v[s] <= v[s+1]) s--;

    cout << s + 1 << endl;
// int prev = INT_MAX; // Initialize prev to a large value
// maxi = 0; // Initialize maxi to an invalid index
...
//             j--;
//         }
//         maxi = (v[maxi] < prev) ? i : maxi; // Update maxi only if the current element is less than prev
//     } else if (v[i] < v[j]) {
//         if (prev > v[i]) {
//             maxi = i;
//         }
...
// The variable 'maxi' now holds the maximum index where v[maxi] is less than 'prev'
 

    // for(int i=1; i+1<n; i++){
    //     int prev = 0, next = 0;
    //     int ii = i, jj = i;
    //     while(ii > 0 && v[i] == v[ii]) ii--;
    //     while(jj < n && v[i] == v[jj]) jj++;
    //     prev = v[ii];
    //     next = v[jj];
    //     if(prev > v[i] && next > v[i]){
    //         maxi = ii + 1;
    //     }
    // }
