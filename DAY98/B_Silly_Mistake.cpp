    ...
    forn(i,n){
        ...
        if(xx < 0){
            if(mp[abs(xx)] == 0) {
                ...
                // }
                mp.erase(abs(xx));
                // cout << "erase kiya" << endl;
                // if(i == 1) {
                //     printm(mp);
                // }
                done[abs(xx)]++;
                // cout << "DONE " << sp << i << sp << xx << endl;
            }

            if(mp.size() == 0){
                ans.pb(i + 1);
                done.clear();
                // cout << "clear hua " << i << sp << xx << endl;
            }
            ...
            if(mp[abs(xx)]) {
                // cout << 2 << sp << xx << sp << i << endl;
                h = 1;
            }
            else {
                if(mp[xx] == 0) mp.erase(xx);
                if(done[xx] > 0 && mp.size() > 0) {
                    // cout << "done " << xx << sp << i << endl;
                    // cout << 3 << endl; 
                    h = 1;
                }
                else {
