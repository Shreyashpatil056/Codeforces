...
void shreyu(){
    ...
    if(n > 1) cout << 1 << sp << n << endl;

    m = (v[0] + v[n-1]) % 2 ? v[0] : v[n-1];

    for(int i=1; i < n-1; i++){
        if((m + v[i]) % 2) cout << 1 << sp << i+1 << endl;
        else cout << i+1 << sp << n << endl;
    }

    // cout << n - 1 << endl;
    // forn(i,n-1){
    //     cout << i+1 << sp << i+2 << endl;
    // }
    
    // 1 100 3 0 5
    // even -> l = r, odd -> r = l;

    // 7 1 30 0 100 5 2 3 4
    // 1 1 30 0 100 5 2 3 4
    // 1 1 1  0 100 5 2 3 4
    // 1 1 1  1 100 5 2 3 4
    // 1 1 1  1 1   5 2 3 4
    // 1 1 1  1 5   5 2 3 4
