...
void rec(v64 &v, int idx, int n, int &bada, int &chota, int temp){
    if(idx == n) return;

    temp = v[idx];
    v[idx] = 0;
    rec(v, idx+1, n, bada, chota, temp);
...
void shreyu(){
    ...
    int bada = 0, chota = 1;
    int temp = 0;
    rec(v, 0, n, bada, chota, temp);

    // my all bigger no's are at right place, but 
    // its not working for smaller numbers
    // so using some jugaad for this
    printv(v);

    int start = 1, end = (n/2)*2 - 1;
    while(start < end){
        swap(v[start], v[end]);
        start += 2;
        end -= 2;
