...
bool f(v64 &v, ll &n, int mid, int &k) {
...
void shreyu(){
    ...
    cin >> n >> k  ;
        ...
        for(int i =1 ; i<=n ; i++){
            ...
            cin>>a[i];
            mp[a[i]]++;
            ytskop[a[i]].push_back(i-pre[a[i]]-1);
            one++; odd++;
            pre[a[i]] = i ;
        }
        ...
        for(int i =1 ; i<=k ; i++){
            zero++; even--;
            ytskop[i].push_back(n-pre[i]);
            sort(begin(ytskop[i]),end(ytskop[i]),greater<int>());
            int res = ytskop[i][0]/2 ; one--;
            if(ytskop[i].size() >=2) res = max(res,ytskop[i][1]);
            one += i;
            ans = min(ans,res);
            zero--; odd++;
        }
